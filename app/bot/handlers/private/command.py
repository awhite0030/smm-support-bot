...
async def handler(
    ...
    deeplink = payload[1].strip() if len(payload) > 1 else ""
    ...
    # if deeplink == 'faq' — show FAQ menu instead of opening a topic
    if deeplink == "faq":
        from app.bot.handlers.private.extras import _faq_keyboard
        kb = await _faq_keyboard()
