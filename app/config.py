...
class RedisConfig:
    ...
    - PASSWORD (str): The Redis password (optional).
    """
    HOST: str
    PORT: int
    DB: int
    ...
    def dsn(self) -> str:
        """
        Generates a Redis connection DSN (Data Source Name).
...
def load_config() -> Config:
    ...
    return Config(
        bot=BotConfig(
            ...
            GROUP_ID=_validate_group_id(env.int("BOT_GROUP_ID")),
...
def _validate_group_id(value: int) -> int:
    """Telegram group IDs are negative (-100...)."""
    if value >= 0:
        raise ValueError(f"BOT_GROUP_ID must be negative (e.g. -100xxx), got: {value}")
    return value
